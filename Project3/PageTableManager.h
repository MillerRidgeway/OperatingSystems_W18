...
public:
   ...
   * getAllocatedFrames()
   * 
   * @returns number of allocated page frames
   */
  int getAllocatedFrames();
  ...
  /**
   * FreeProcessPages - free pages a process has allocated 
   * @param vaddr virtual address to start at
   * @param count count of pages to free
   */
  
  void FreeProcessPages(mem::Addr vaddr, size_t count);
 ...
 private:
   ...
   int allocatedCount; //Number of allocated page frames
