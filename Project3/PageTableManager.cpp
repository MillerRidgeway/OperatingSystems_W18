...
void PageTableManager::MapProcessPages(Addr vaddr, size_t count) {
  ...
  if (!already_mapped.empty()) {
    allocatedCount -= already_mapped.size(); 
...
void PageTableManager::FreeProcessPages(mem::Addr base, size_t count){
  // Check for valid virtual address
  if ((base & mem::kPageOffsetMask) != 0) {
    throw std::runtime_error("vaddr not a multiple of page size");
  }
  ...
  std::vector<Addr> allocated;
  
  //Get current page table entry
  PageTable pt;
  memory.get_bytes(&pt, base, sizeof(PageTable));
  
  //Create list of allocated pages
  for(Addr i = 0; i < mem::kPageTableSizeBytes; i++){
      if(pt[i] != 0){
          std::cout << "Deallocating page..." << std::endl;
        allocated.push_back(pt[i] & mem::kPageTableIndexMask);
      }
  }
  allocatedCount -= allocated.size();
  
  //Include page table base
  allocated.push_back(base);
  
  //Free frames w/ memory manager
  allocator.FreePageFrames(count, allocated);
}
...
int PageTableManager::getAllocatedFrames(){
